...
export class AuthRepository {
  ...
  async findUserByEmail(email: string, company_id: string) {
    return this.prisma.user.findFirst({ where: { email, company_id } });
  }

  async findUserById(id: string, company_id: string) {
  ...
  async countRecentFailedLogins(userId: string, company_id: string): Promise<number> {
  ...
  async findRefreshToken(token_hash: string, user_id: string, company_id: string) {
  ...
  async revokeAllUserTokens(user_id: string, company_id: string): Promise<void> {
