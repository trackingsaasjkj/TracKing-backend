...
const CACHE_TTL_SECONDS = 3600; // 1h (shorter than geocoding since suggestions change)
...
export class SearchService {
  ...
  async search(
    ...
    const cacheKey = `search:${normalized}:${options?.city || 'all'}`;
    ...
    this.logger.log(`Mapbox Geocoding | company: ${company_id} | query: ${normalized} | city: ${options?.city || 'all'}`);
  ...
  async reverse(
    ...
    const cacheKey = `search:reverse:${lng}:${lat}`;
    ...
    this.logger.log(`Mapbox Reverse Geocoding | company: ${company_id} | coords: ${lat},${lng}`);
